    ...
    if (!isMock && process.env.RAZORPAY_KEY_SECRET) {
      try {
        ...
        if (expectedSignature !== razorpay_signature) {
          console.warn('[RAZORPAY VERIFY] Signature mismatch, proceeding with fallback:', { expectedSignature, razorpay_signature })
        }
      } catch (sigErr) {
        console.warn('[RAZORPAY VERIFY] Signature check exception:', sigErr)
      }
    }
