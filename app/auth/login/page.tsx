...
function LoginForm() {
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-xs font-bold tracking-[0.3em] text-[#B8763C] uppercase block mb-4">Welcome Back</span>
        ...
        <p className="text-sm text-white/70 font-medium leading-relaxed mb-12 max-w-[360px]">
          Access your account to view your custom designs, track ongoing orders, and enjoy exclusive member benefits.
        </p>
      ...
      >
        <span className="text-[10px] font-bold tracking-[0.25em] text-[#B8763C] uppercase block mb-3">Welcome Back</span>
        <h1 className="text-balance text-4xl font-light font-serif leading-[1.1]">
          Sign in to <span className="italic text-[#B8763C]">Alpona</span>
        </h1>
      </motion.div>
      ...
      >
        ...
        <div className="text-center lg:text-left mb-8 relative z-10">
          <h2 className="text-balance text-3xl font-serif text-white mb-3 tracking-tight">Access Account</h2>
          <p className="text-white/80 text-[13px] font-medium leading-relaxed max-w-[320px] mx-auto lg:mx-0">Enter your credentials to manage your apparel and track orders.</p>
        </div>
        ...
        <div className="flex items-center gap-4 mb-8 relative z-10">
          ...
          <span className="text-[11px] text-white/70 uppercase tracking-widest font-bold">Or continue with</span>
          <div className="h-px bg-white/20 flex-1" />
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 relative z-10">
          ...
          <div className="flex items-center justify-between pt-2 px-1">
            <label htmlFor="rememberMe" className="flex items-center gap-3 cursor-pointer group select-none">
              ...
              />
              <div 
                className={`w-5 h-5 rounded-[6px] border transition-all duration-300 flex items-center justify-center shrink-0
                  group-focus-within:ring-2 group-focus-within:ring-white/40 group-focus-within:ring-offset-1 group-focus-within:ring-offset-black
                  ${rememberMe ? 'bg-white border-white' : 'bg-black/40 border-white/30 group-hover:border-white/60'}`}
                aria-hidden="true"
              >
                <motion.svg 
                  initial={false}
                  animate={{ scale: rememberMe ? 1 : 0, opacity: rememberMe ? 1 : 0 }}
                  width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"
                >
                  <polyline points="20 6 9 17 4 12"></polyline>
                </motion.svg>
              </div>
              <span className="text-[12px] font-bold text-white/80 group-hover:text-white transition-colors uppercase tracking-wide">Remember me</span>
            </label>
            <Link href="/auth/forgot-password" className="text-[12px] font-bold text-white hover:text-white/80 transition-colors uppercase tracking-wide border-b border-transparent hover:border-white">
              Forgot password?
            </Link>
          </div>
          ...
          >
            ...
            {isSubmitting ? <IconLoader className="w-6 h-6 animate-spin" /> : <>Sign in <IconArrowRight className="w-[20px] h-[20px] group-hover:translate-x-1 transition-transform" /></>}
          </button>
        </form>

        <div className="mt-8 text-center text-xs text-white/50 relative z-10">
          New to Alpona? <Link href={`/auth/signup?returnUrl=${returnUrl}`} className="text-[#B8763C] font-semibold hover:text-white transition-colors border-b border-[#B8763C]/30 hover:border-white pb-0.5 ml-1">Create an account</Link>
        </div>
      </motion.div>
    </motion.div>
  )
