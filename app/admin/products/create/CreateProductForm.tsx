...
export function CreateProductForm({ categories, subcategories, productTypes, availableDesigns = [], redirectPath = '/admin/products' }: CreateProductPageProps) {
  const router = useRouter()
  const [productType, setProductType] = useState<'pod' | 'manual' | null>(null)
  ...
  const [productCareInfo, setProductCareInfo] = useState('')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.set('qikink_product_id', productType === 'pod' ? qikinkProductId : '')
    formData.set('is_active', isActive.toString())
    formData.set('images', JSON.stringify(images))
    
    // Process colorImages into variants
  ...
  // Step 1: Select Fulfillment Type
  if (!productType) {
    return (
      <div className="max-w-4xl mx-auto py-8 space-y-8">
        <div>
          <Link href={redirectPath} className="text-xs text-zinc-400 hover:text-white flex items-center gap-1.5 mb-3 transition-colors">
            <ArrowLeft className="w-3.5 h-3.5" /> Back to Products Catalog
          </Link>
          <h1 className="text-2xl font-black text-white tracking-tight">Create New Product</h1>
          <p className="text-zinc-400 text-xs mt-1">Select the fulfillment workflow for this catalog item.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <button
            onClick={() => setProductType('pod')}
            className="group relative bg-[#121214] border border-zinc-800 hover:border-blue-500/50 rounded-2xl p-8 text-left transition-all duration-300 hover:shadow-2xl hover:shadow-blue-500/10 cursor-pointer overflow-hidden"
          >
            ...
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-lg font-bold text-white group-hover:text-blue-400 transition-colors">Qikink (Print on Demand)</h3>
              <span className="text-[10px] font-bold uppercase tracking-wider bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2 py-0.5 rounded-full">Automated</span>
            </div>
            <p className="text-zinc-400 text-xs leading-relaxed">
              Orders automatically sync with Qikink for printing, packaging, and direct customer delivery. Zero upfront inventory required.
            </p>
          </button>

          <button
            onClick={() => setProductType('manual')}
            className="group relative bg-[#121214] border border-zinc-800 hover:border-amber-500/50 rounded-2xl p-8 text-left transition-all duration-300 hover:shadow-2xl hover:shadow-amber-500/10 cursor-pointer overflow-hidden"
          >
            ...
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-lg font-bold text-white group-hover:text-amber-400 transition-colors">Manual Inventory</h3>
              <span className="text-[10px] font-bold uppercase tracking-wider bg-amber-500/10 text-amber-400 border border-amber-500/20 px-2 py-0.5 rounded-full">Self-Fulfilled</span>
            </div>
            <p className="text-zinc-400 text-xs leading-relaxed">
              For items stocked in your warehouse or local inventory. Orders trigger standard manual dispatch and tracking updates.
            </p>
          </button>
        </div>
      </div>
  ...
  // Step 2: Full 2-Column Professional Form Layout
  return (
        ...
        <div className="max-w-[1400px] mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button
              ...
              title="Change Fulfillment Type"
            >
              <ArrowLeft className="w-4 h-4" />
            </button>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-base font-bold text-white">Create New Product</h1>
                <span className={`px-2 py-0.5 rounded-md text-[10px] font-extrabold uppercase tracking-wider border ${
                  productType === 'pod' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'
                }`}>
                  {productType === 'pod' ? 'Qikink POD' : 'Manual Stock'}
                </span>
              </div>
              <p className="text-[11px] text-zinc-400 truncate max-w-md">
                {name || 'Untitled Product'} {generatedSlug ? `• /shop/${generatedSlug}` : ''}
              </p>
            </div>
          </div>
          ...
          <div className="bg-[#121214] border border-zinc-800/80 rounded-2xl p-6 space-y-5">
            ...
            <div className="space-y-1.5 bg-[#09090b] p-3 rounded-xl border border-zinc-800/60">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-zinc-500 font-mono">/shop/{generatedSlug || 'your-product-slug'}</span>
              </div>
              <Input
                value={customSlug}
                onChange={(e) => setCustomSlug(e.target.value)}
                placeholder="Custom slug (leave empty to auto-generate from title)"
            ...
            {variants.length === 0 ? (
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto pr-1">
                <div className="grid grid-cols-12 gap-2 text-[10px] font-bold text-zinc-400 uppercase tracking-wider px-3">
                  ...
                  {productType === 'pod' && <span className="col-span-4">Qikink SKU</span>}
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-[#121214] border border-zinc-800/80 rounded-2xl p-5 space-y-4 sticky top-24">
            ...
            <div className="space-y-3 pt-2">
              <h4 className="text-[11px] font-extrabold uppercase tracking-wider text-zinc-400">Catalog Organization & Gender Fit</h4>
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(val: string | null) => {
                    ...
                    // Auto update category if unassigned
              ...
              {/* Target Audience / Gender Fit Selection (Both Men & Women support) */}
                ...
                <div className="flex items-center gap-1.5">
                  {profitMetrics.profit > 0 && (
                    <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded ${
                      ...
                      {profitMetrics.margin}% (₹{profitMetrics.profit})
                    </span>
                  )}
                  <button
                    type="button"
                    onClick={async () => {
                      ...
                      try {
                        ...
                        toast.error('Error connecting to AI Pricing API', { id: tid })
                      }
                    }}
            ...
            <div className="p-3.5 rounded-xl bg-[#09090b] border border-zinc-800/60 space-y-2">
